   ...
   public function updateProfile(Request $request)
    ...
    public function store_site_images(Request $request){
        ...
        if(count($errorImages) > 0){
            ...
            // ============================================================
            // ===========  delete expire link images from all  ===========
            // ============================================================
            
            
            $todayDate = date('Y-m-d');
            
            $list = UserSites::where('end_date', '<' , $todayDate)->where('status', 'active')->get();
            if(count($list) > 0){
                foreach ($list as $li) {
                    $usersite_id = $li->id;
                    if ($li->delete()) {
                            $listmedia = UsersitesMedia::where('usersite_id', $usersite_id)->get();
            ...
            $todaydate = date('Y-m-d');
            $valid_days = 30;
            $end_date = date('Y-m-d', strtotime($todaydate . ' +'.$valid_days.' days'));
    ...
    public function email_to_sent_link(Request $request)
    {
        ...
        if($usersitedata->sitetype == 'Publisher'){
            $message1 = $usersitedata->preview_link;
            $message2 = $usersitedata->preview_link;
        }else{
        ...
        Mail::send('email.preview_link', $email_data, function ($message) use ($email_data, $sub1) {
            $message->to($email_data['email'], $email_data['name'])
                ->subject($sub1)
                ->from('info@danish.dbtechserver.online', "The Seller's Friend");
        });
        
        $succ_msg = "Preview links for ".$usersitedata->sitetype." sent successfully. Please check your email for same.";
        return redirect()->back()->with('success_email', $succ_msg);
